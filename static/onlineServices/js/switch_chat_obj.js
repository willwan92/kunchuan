...
function updateSessDiv(sess_type, to_id, name, unread_msg_count) {
    ...
    var sig = item.split('=')[2];
    obj[key] = value;
    queryArr.push(obj);
    })
     // 本地  url http://localhost:8080/api/gym/Im/getProfilePortraitGet
    //测试  url  http://api-control.test.hxsapp.com/gym/Im/getProfilePortraitGet
    $.ajaxSettings.async = false;
    $.post("http://api-control.hxsapp.com/gym/Im/getProfilePortraitGet",{
        usersig:queryArr[2].sig,
        accountList:to_id
    },
    function(res){
        ...
        console.log("你好",res)
        if(result.code == 200){
            fromAccountNick = result.data.UserProfileItem[0].ProfileItem[0].Value;
            fromAccountImage = result.data.UserProfileItem[0].ProfileItem[1].Value;
        }
    })
    ...
    if (badgeDiv && unread_msg_count > 0) {
        ...
        addSess(sess_type, to_id, fromAccountNick, headUrl, unread_msg_count, 'sesslist');
    }
}
...
function addSess(sess_type, to_id, name, face_url, unread_msg_count, sesslist, addPositonType) {
    ...
    if (!addPositonType || addPositonType == 'TAIL') {
        ...
        console.log('12121',webim.Log.error('未知addPositonType' + addPositonType));
    }
}
